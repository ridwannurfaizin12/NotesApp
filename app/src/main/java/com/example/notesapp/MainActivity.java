...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        simpanCatatan.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Data data = new Data();
                data.setJudul(judulCatatan.getText().toString());
                data.setCatatan(isiCatatan.getText().toString());

                boolean masukkanCatatan;
                if (edit) {
                    masukkanCatatan = dbHelper.updateNotes(data, id);
                } else {
                    masukkanCatatan = dbHelper.inputNotes(data);
                }
                if (masukkanCatatan) {
        ...
        //Tombol membatalkan pembuatan atau penyuntingan catatan
    ...
    private void dialogDaftarCatatan () {
        ...
        View v = inflater.inflate(R.layout.layout_list_notes, null);
        TextView hapusSemuaCatatan = v.findViewById(R.id.hapusSemuaCatatan);
        TextView judulDaftar = v.findViewById(R.id.judulDaftar);
        b.setView(v);

        ArrayList<Data> setterGetterData = new ArrayList<>();
        ListView listDaftar = v.findViewById(R.id.listDaftar);
        final DBHelper dh = new DBHelper(getApplicationContext());
        Cursor cursor = dh.allNotes();
        cursor.moveToFirst();
        if (cursor.getCount() > 0) {
            while (!cursor.isAfterLast()) {
                Data data = new Data();
                data.setId((cursor.getInt(cursor.getColumnIndexOrThrow("id"))));
                data.setJudul((cursor.getString(cursor.getColumnIndexOrThrow("judul"))));
                data.setCatatan((cursor.getString(cursor.getColumnIndexOrThrow("catatan"))));
                setterGetterData.add(data);
                cursor.moveToNext();
            }
            dh.close();
        }
        ...
        AdapterDaftarNotes adapterDaftarCatatan = new AdapterDaftarNotes(setterGetterData, getApplicationContext(),
                daftarCatatan);
